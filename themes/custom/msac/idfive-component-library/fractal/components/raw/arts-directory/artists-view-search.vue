<template>
  <form class="arts-directory__view-search" v-on:submit.prevent="handleSubmit($event)">
    <div class="form-item">
      <label for="artists-view-search-input">Search by Keyword</label>
      <input
        class="arts-directory__search-input text-input"
        type="text"
        id="artists-view-search-input"
        v-model="viewState.keywordSearchValue"
        ref="input"
      />
    </div>
    <div class="form-item">
      <label for="artists-view-county-select">Located In</label>
      <select
        class="select"
        id="artists-view-county-select"
...
import { state as viewState, actions as viewActions } from "./artists-view-store";
