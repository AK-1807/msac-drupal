<template>
  <form class="arts-directory__view-search" v-on:submit.prevent="handleSubmit($event)">
    <div class="form-item">
      <label for="organizations-view-search-input">Search by Keyword</label>
      <input
        class="arts-directory__search-input text-input"
        type="text"
        id="organizations-view-search-input"
        v-model="viewState.keywordSearchValue"
        ref="input"
      />
    </div>
    <div class="form-item">
      <label for="organizations-view-county-select">Located In</label>
      <select
        class="select"
        id="organizations-view-county-select"
...
import { state as viewState, actions as viewActions } from "./organizations-view-store";
