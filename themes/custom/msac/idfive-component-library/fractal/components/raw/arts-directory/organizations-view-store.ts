...
export const actions = new ViewActions(state, "/node/organization", () => ({
  include: "field_org_image,field_org_services,field_org_services.field_services_items,field_org_artistic_categories",
  ...getFilterParams(),
  ...getSortParams(),
}));
...
function getFilterParams() {
  ...
  if (artisticCategoryIds.length) {
    params["filter[artistic-cat][condition][path]"] = "field_org_artistic_categories.id";
    params["filter[artistic-cat][condition][operator]"] = "IN";
    params["filter[artistic-cat][condition][value]"] = artisticCategoryIds;
  }

  if (state.filters.aeDistrict.value) {
    params["filter[field_org_districts.id]"] = state.filters.aeDistrict.value;
  }

  if (state.filters.bookingPrice.value) {
    params["filter[field_org_bk_price.id]"] = state.filters.bookingPrice.value;
  }

  if (state.filters.presentingTouring.value) {
    params["filter[field_org_presenting_touring]"] = "1";
  }

  // If the option to show all orgs with AEI filters is selected,
  ...
  if (state.selectAllArtsInEducationFilters || state.filters.populationTypes.value.length) {
    ...
    params["filter[population-type][condition][path]"] = "field_org_population_types.id";
  ...
  for (let i = 0; i < engagementTypes.length; i++) {
    if (state.selectAllArtsInEducationFilters || state.filters[engagementTypes[i]]?.value) {
      ...
      params[`filter[${filterName}][condition][path]`] = `field_org_engmt_type_${i + 1}`;
